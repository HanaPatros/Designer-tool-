	...
	ActionListener listener = new ActionListener() {
		...
		public void actionPerformed(ActionEvent event) {
			if (event.getSource() == clearButton) {
				...
				canvas.pencil();
                                //test = canvas.getSvg().get;
                                 StringBuilder result = new StringBuilder();
                               for (int i = 0; i < canvas.getSvg().size(); i++) {
                                    result.append(canvas.getSvg().get(i));
                            }
                               coordinaten.setText(result.toString());
			} 
		}
	};
...
//coordinaten.setText(String.valueOf(test));
